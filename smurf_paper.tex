...
large-scale anisitropies in the Cosmic Microwave Backgroubd.  Styles
...
designed to use a less accurate (primarily on large angular scales),
...
and then regrid the residual signals. This method is similar in spirit
to the Comprehensive Reduction Utility for SHARC-2
...
$T_\mathrm{C}$. The TESs then have a small electrical voltage bias,
and individual pixel heaters (using a current bias to apply the
desired Ohmic heating) are used to place the detectors in the
...
of 40 detectors. Each of the 32 rows is then read-out in sequence that
occurs at a frequency of ...kHz?... This multiplexing scheme greatly
...
The focal planes at each of 450 and 850\,\micron\ are populated with
four of these $40 \times 32$ subarrays (referred to as s4a--s4d, and
s8a--s8d respectively), and simultaneously image approximately the
same region of the sky by means of a beam-splitter.
