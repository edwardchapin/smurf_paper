...
In this example we produce a map of Uranus, which is a nearly
point-like source for SCUBA-2 that is commonly used as a primary flux
calibrator (observation 26 on 20111017). The first reduction uses the
defaults as described in Section~\ref{sec:algorithm}, but we manually
set the number of iterations to a fixed value of 100 to illustrate how
the solution converges in the absence of any boundary conditions.
...
\caption{Multiple reductions of a 450\,\micron\ daisy scan of Uranus,
  ...
  is not removed by the common-mode; (b) same as (a) but using 50
  iterations, illustrating that there is a degeneracy between
  large-scale structure and the common-mode (the footprint of working
  bolometers is also shown for reference); (c) reduction in which
  high-pass filtering above 0.775\,Hz (corresponding to a spatial
  scale of 200\,arcsec as indicated) is applied after common-mode
  removal, but before the map estimate --- the map-based convergence
  test is activated and the solution halts after 10 iterations, but
  leaving large-scale ringing due to the filter; (d) same as (c), but
  now the region of the map beyond the white dot-dashed circle is
  ...
  source flux, and the diffraction pattern is clearly seen.}
\label{fig:pointmaps}
\end{figure*}
