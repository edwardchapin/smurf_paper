...
The accurate measurement of positions and brightnesses of point
...
degeneracies in the map solution.

To illustrate how large-scale ripples can form (and in fact, diverge),
the same map solution is run for 50 iterations and shown in
...
\model{AST} signals are \emph{complimentary}. In other words, the large
...
One simple improvement is to simply restrict the low-frequency signal
that goes into the maps using a high-pass filter. A third reduction of
the data uses the default map-making parameters, as described in
...
0.78\,Hz. Now using the automatic map-based convergence test (Section
???), the resulting map converges after 10 iterations
(Fig.~\ref{fig:pointmaps}c). Both the circular streaks and the
large-scale gradient are now removed, but they have been replaced by
an obvious, circularly-symmetric ringing pattern about Uranus. The
reason for this ringing is due to the fact that the hard-edged
...
source is actually removed for subsequent iterations of
\model{FLT}, but as in the previous case, generacies between the
map and the \model{FLT} and \model{COM} signal components can still
lead to complicated structures.

The preceding examples illustrate the need for prior constraints in
the map solution. For calibrators (and other previously known bright,
compact sources), a good, simple prior is to constrain the map to a
value of zero away from the known locations of emission. In
Fig.~\ref{fig:pointmaps}d, a map is produced in an identical manner to
the previous example, but now setting the map explicitly to zero
beyond a radius of 60\,arcsec from the location of Uranus (much larger
than the FWHM of the main lobe), for all but the final iteration. In
this case, the map converges after 22 iterations, and all of the
previous artifacts have been effectively removed. Since the map is now
flat away from the source, and constrained to a value of zero, it is
now appropriate for performing aperture photometry directly, with no
need for an additional reference annulus. This approach to constrained
...
\caption{Multiple reductions of a 450\,\micron\ daisy scan of Uranus,
  ...
  as (a) but using 50 iterations, illustrating that there is a
  ...
  peak 0.14\,pW); (c) reduction in which high-pass filtering above
  ...
  due to the filter (Uranus peak 0.38\,pW); (d) same as (c), but now
...
(e.g. high-redshift star-forming galaxies, and debris disks) need to
obtain the best noise performance possible. The approach described
here involves three major steps: (i) generating a map that removes
most low-frequency noise sources; (ii) \emph{whitening} (essentially
flattening) the map to make the noise properties of the map easier to
deal with; and (iii) detecting point sources using a
\emph{matched-filter}. Note that variations on this general procedure
have been used extensively in the submm cosmology community using
previous instruments \citep[e.g.][]{scott2008,perera2008}.  In this
section we will reduce data of the Lockman Hole taken in 2010 during
...
15\,min. each) spread over February and March, covering an area of
about 50\,arcmin$^2$. ... full obs list ... dropped scans due to
terrible noise properties ...

The first step, map generation, is accomplished in a way similar to
the default described in the previous section (without any boundary
constraints)...

Even though the map \emph{looks} extremely flat, it is clear that
there is a mixture of faint astronomical sources, and what is probably
residual low-frequency noise causing faint ripples. With uncertainties
about what is source, and what is an artifact, the first step is to
remove the low-frequency noise in a way that does not affect
(appreciably) the real sources. To accomplish this goal, the map is
run through a whitening filter that results in a flat angular noise
power spectrum. The noise power spectrum is estimated from a
\emph{jackknife map}: maps are produced from two independent halves of
the total data set, and then a pixel-by-pixel difference is
calculated. Provided that the noise in one half of the data is
uncorrelated with that from the other half, the noise in the jackknife
map should resemble that of the total map, though larger by a factor
of $\sim\sqrt(2)$ depending on how closely the pixel variances in the
two maps are matched (if one map pixel is significantly noisier, it
will dominate the noise in the jackknife).  The astronomical signal,
however, should be cleanly removed (provided that there are no strong
time-varying signals, and also assuming that errors due to calibration
between the two halves are insignificant). The approach we have taken
to minimize systematics is to produce two halves of the jackknife
using odd and even scan numbers (i.e., each map will contain an even
sampling of data across the full dataset).

\begin{figure*}
\centering
\includegraphics[width=\linewidth]{lockman_maps.pdf}
\caption{Procedure for producing maps of deep fields.}
...
\caption{Azimuthally-average angular power spectra.}
...
\caption{Filtering effects on PSF.}
...
symmetric (inspection of the 2-dimensional FFT confirms
this). ... calculate filter as slightly-smoothed complement of
azavpspec ... Finally, the filter is applied to the map, resulting in
panel...
