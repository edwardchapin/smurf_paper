...
map does not lie near any radio source.

Note that for significantly deeper future SCUBA-2 maps, in which the
RMS in a PSF-smoothed map is dominated by point-source confusion,
rather than instrumental noise, a modified matched-filter will offer
improved results. See Appendix~A in \citet{chapin2011}, which shows
how to include confusion (when known \emph{a priori}) explicitly as a
noise term in the calculation of such filters.

\subsection{Bright extended emission}
The hardest thing to do with \scuba\ at the moment. Try to generate a
