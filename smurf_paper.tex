...
\begin{abstract}
  ...
  User Bolometer Array 2 (\scuba), a camera for the 15-m James Clerk
...
like \scuba\ are limited by photon noise from the instrument and
ambient backgrounds. The low-frequency noise, however, is dominated by
sources such as slow variations in mean backgrounds (e.g., thermal
variations within the cryostat, and the atmosphere for ground-based
cameras), and drift in the readout electronics. Such noise has a power
spectrum $\propto 1/f^\alpha$ ($\alpha>0$), and the frequency at which
it is comparable to the white noise level is called the ``$1/f$
knee''. Since the low-frequency noise is largely correlated between
all, or subsets, of the bolometers in time, it can be suppressed
during map-making, since astronomical signals have the distinct
property that they are fixed in a sky reference frame (assuming they
are not time-varying). If successful, the noise in the resulting map
is said to be ``background noise limited'', meaning that the map noise
...
600\,arcsec\,sec$^{-1}$ slew speeds); and (ii) by offering scan
patterns that cross the sky at a wide range of position angles,
ultimately observing each portion of the map at a number of different
times. Such methods are now used by virtually all existing
...
angular scales
\citep[e.g.][]{glenn1998,weferling2002,laurent2005,wilson2008,kovacs2008b}.

There are three general styles of map-making that are described in the
literature for bolometer data. The best maps are produced using
\emph{maximum likelihood} techniques, in which the map is expressed as
a linear combination of the data using weights that are a function of
the time-domain noise properties of the bolometers. The first good
description of this method appears in \citet{janssen1992} for the
production of maps from the COsmic Background Explorer
(COBE). However, this method can be extremely computationally
expensive, even when small numbers of bolometers are involved,
although we note that an algorithm called ``SANEPIC'' for reducing
maps from the Balloon-borne Large-Aperture Submillimeter Telescope
(BLAST) involving hundreds of bolometers and correctly incorporating
inter-bolometer noise correlations, has been implemented with
reasonable execution speeds and memory requirements
\citep[][]{patanchon2008}.
