...
low-frequency noise sources with approximately linear response without
prior knowledge of the location of sources; (ii) \emph{whitening}
...
In this section we will reduce data of the Lockman Hole taken in 2010
...
area of about 50\,arcmin$^2$. ... full obs list ... dropped scans due
to terrible noise properties ...
...
by the solid and dotted lines in Fig.~\ref{fig:psf},
respectively. Clearly the down-side of using a high-pass filter as a
pre-processing step, and having no other map-constraints, has the
down-side of introducing sidelobes around the main peak. ... high-pass
...
\sigma^2_\mathrm{JK} = \frac{\sigma^2_1 + \sigma^2_2}{2}
\end{eqnarray}


Provided that the noise in one half of the data is uncorrelated with
that from the other half, the noise in the jackknife map should
resemble that of the raw map. The astronomical signal, however, should
be cleanly removed (provided that there are no strong time-varying
signals, and also assuming that errors due to calibration between the
two halves are insignificant). The approach we have taken to minimize
systematics is to produce two halves of the jackknife using odd and
even scan numbers (i.e., each map will contain an even sampling of
data across the full dataset).
...
\caption{Azimuthally-averaged angular power spectral densities for the
  raw map output by SMURF (dashed black line), the jackknife (solid
  orange line), and whitened (solid black line) maps (panels (a), (d),
  and (b) from Fig.~\ref{fig:lockman_maps}, respectively. Since the
  raw map contains spatially correlated signals on large scales both
  due to noise and astronomical signals (low-spatial frequencies), the
  jackknife map (difference of two approximately equal-length subsets
  of the data) is used to generate a plausible realization of pure
  noise. Assuming that the noise properties are isotropic, a whitening
  filter is estimated from the reciprocal of the jackknife power
  spectrum with only a radial dependence. The power spectrum of the
  resulting signal map has residual power on large scales which is
  presumably due to astronomical sources.}
