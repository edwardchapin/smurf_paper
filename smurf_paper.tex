...
Fig.~\ref{fig:steps1} shows such steps in the time streams for two
bolometers. For each bolometer, correcting these steps involves:

\begin{enumerate}

\item Locating each candidate step.

\item Determining the widths of the candidate steps. Some steps
...
at the bottom of the step in the right hand plot of
Fig.~\ref{fig:steps1}.
...
\paragraph{Detecting Candidate Steps:}
...
indices 5040, 5100 and 5280. In addition, the varying gradient of the
underlying noise-free time series can be seen in the lower level changes
in the difference values.
...
times the RMS of the neighbouring difference value. However, the RMS of
the differences can be heavily affected by the gradient of the underlying
noise-free time series. If there is a strong upward or downward trend in
the time series data, the RMS of the difference values will be higher
potentially resulting in some steps being missed. Therefore to enhance
the reliability of the step detection, an attempt is made to remove these
variations before locating the steps. To do this, the differences are
smoothed using mean filter, and the smoothed differences are removed. The
remaining residual difference values represent the gradient due to steps,
spikes, noise, etc. It is important that the smoothed differences are not
polluted by the large values caused by spikes and steps. So before
smoothing, differences of more than three times the RMS difference (taken
over the whole time stream) are identified and flagged. The RMS is then
re-calculated omitting the flagged values and a further two such
rejection iterations are performed. This flags high gradient values
