...
Fig.~\ref{fig:m17}, after 1, 30 iterations (by this last
iteration the map-based convergence criterion was achieved). The first
panel also depicts the array footprint, and the angular scale
corresponding the high-pass filter edge. Much like the reduction of a
point source without any prior constraints
...
may not be possible for the observer to define, in advance, a mask to
define regions of source and blank sky. Indeed, for this map, much of
the field clearly contains extended structures. Furthermore, the goal
of such maps may be to detect \emph{previously unknown} cool, dense
regions of the ISM that may not have appeared at other wavelengths
(e.g. the first optically-thick cloud-collapse stages of
star-formation). While the option does exist for the user to supply
their own mask, a facility has been added to SMURF to generate one
automatically by flagging pixels above some SNR threshold after each
iteration and is referred to as the ``bright extended'' configuration.

The results of this automatic masking are shown in the bottom panels
of Fig.~\ref{fig:m17}. After the first iteration, only narrow regions
...
encompasses by the mask.

Note that alternatives to this approach do exist for other iterative
map-makers. For example, \citet{kovacs2008} restricts the solution to
a small number of iterations to avoid divergence on large-scales. For
BOLOCAM data of the Galactic Plane, \citet{aguirre2010} used a
Maximum-Entropy filtering step to suppress large-scales. The
zero-masking implemented in SMURF is similar in principle to the
\citet{aguirre2010}, while being slightly conceptually
simpler. However, since both methods are non-linear, detailed
simulations would be required to determine the SNR-dependent transfer
function.
...
maps and their response to real structures. Similar to the previous
section, we will use a jackknife test to estimate the noise, and then
inject known sources into the data to observe how they are attenuated.
...
Next, we inject simulated clouds into a relative empty portion of the
observation 11 M17 map. It is created by filtering white noise first
with a power-law index $alpha=$bla (a reasonable approximation for
Galactic Cirrus... cite bla bla), and then with a 14.5\,arcsec
Gaussian to model the effect of the SCUBA-2 optical response. Finally,
the signal is apodized ... Compare power-spectrum before and after.
