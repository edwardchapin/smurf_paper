...
common-mode removal corrects it to a large extent. Part of the reason
...
\ref{fig:pspec}b. The 12 most significant components are shown (ranked
...
that resembles the scan pattern in Fig.~\ref{fig:bolos_mix}b. While
these correlated signals also clearly appear at both wavelengths,
referring to the maps of eigenvalues, they are considerably stronger
at 850\,\micron\ (consistent with the visual appearance of the
bolometer signals in Fig.~\ref{fig:pca}). Also note that the PSDs for
these eigenvectors exhibit nearly a pure $1/f$ signature with almost
no line features, or a white-noise plateau (suggesting that these are
high-\snr\ measurements of a purely low-frequency drift).
...
values near $\sim$0.85 are common, depending primarily on the fraction
of the total bandwidth that has been removed by high-pass filtering
(Section~\ref{sec:flt}). Regardless of the normalisation, once the
model parameterisation has been chosen, this quantity should converge
to some fixed value.
...
While most of the steps described in the Section~\ref{sec:algorithm}
for reducing data with SMURF are applicable to all data sets, there is
no ``default'' reduction that provides good results for a wide range
of source \snr\ and angular scales. However, SMURF does have a small
