...
already used extensively at the Joint Astronomy Centre (host of the
JCMT) for many other systems, which helps with
...
In Sections~\ref{sec:dataprep} and \ref{sec:components} we describe
the data pre-processing steps and iterative algorithm in
detail. Section~\ref{sec:converge} explores convergence tests and
degeneracies between model components.
...
Prior to executing the iterative part of the SMURF algorithm, the data
must undergo several pre-processing steps. First, the data files are
read into memory and concatenated into continuous arrays (\scuba\ data
are broken up and written to disk every $\sim$30\,s). As the data are
loaded, the flatfield correction is also applied \citep[see
Section~2.1 in][]{dempsey2012}. Next, a series of data cleaning and
...
the frequency domain. Such behaviour is desirable since it serves as
an anti-aliasing filter; most non-white features above the Nyquist
...
ringing in the FLT model (Section~\ref{sec:flt}) and visible streaks
in the final map, corresponding to the paths of individual bolometers
over the sky.
...
in the map. This procedure is summarized in Fig.~\ref{fig:dimm}.
 ...
 and run through a pre-processing stage that performs
