...
\begin{abstract}
  ...
  User Bolometer Array 2 (\scuba) taken during the Shared Risk
...
such as \scuba, is limited by photon noise from the instrument and
...
to-date with \scuba, we note that the \scuba\ Shared-Risks Observing
...
For future, significantly deeper SCUBA-2 maps, in which the RMS in a
...
array. It is a rotating PONG convering a diameter of 0.375\,deg, with
a scan speed of 300\,arcsec\,s$^{-1}$, and a transverse spacing of
...
the noise weights have been measured) and 30 iterations (when the map
has converged). The first panel also depicts the array footprint, and
the angular scale corresponding the high-pass filter edge. Much like
the reduction of a point source without any prior constraints
...
\begin{figure*}
\centering
\includegraphics[width=0.8\linewidth]{m17_jk.pdf}
\caption{Maps of M17 used to characterize the noise properties and
transfer function of SMURF. For each column, a different high-pass
filter edge scale was adopted (indicated in the top panels). First
(top) row: Average of two halves of the data analyzed independently,
using the default configuration. The data have had added to them
synthetic signal within a 600\,arcsec diameter circle (S.E. of M17)
created as a realization of noise from a $P(k) \propto k^{-3}$ angular
power spectrum multiplied by the PSF, subtracting the minimum to make
it positive, scaling it to a similar signal range as M17 itself, and
...
scales. The red, green, and blue colours indicate data filtered at
150, 300, and 600\,arcsec scales. right: corrected PSDs. Since the
input PSD is known, it is possible to measure the transfer function of
the map-maker as the ratio between the output map signal and input
PSDs (thick solid lines). The remaining lines are as in the left
panel, but now corrected by the transfer function. This plot shows
that while increasing the filter scale improves the SNR at large
scales ($\gsim100$\,arcsec), it also reduces the SNR at smaller scales
($\lsim100$\,arcsec).}
...
would also like to know these are affected by our choice of filter
scale.  Similar to the previous section, we will use a jackknife test
to estimate the noise, and then inject known sources into the data to
observe how they are attenuated.
...
distribution from the jackknife. Also, since the zero-masking depends
on the SNR of the map, it will be restricted to regions approximately
$\sqrt{2}$ shallower in these maps than for the full data
set. Finally, the cross-linking (positional angles sampled) is
...
%A second map of M17 using the same observing mode was made shortly
%after the first, observation 13 on 20110511. We produce jackknife maps
%from the differences between the two ... angular power spectrum of the
%noise (should be worse at large-scale for the default map).

In addition to dividing the data into two halves, we also inject
simulated clouds into a relative empty portion of the map. It is
created by drawing a realization of noise within an 800\,arcsec
on-a-side box, from an angular power spectrum $P(k) \propto k^{-3}$
which is appropriate for Galactic cirrus clouds
\citep[e.g.][]{gautier1992}. It is then filtered again with a
14.5\,arcsec Gaussian to model the effect of the SCUBA-2 optical
...
columns show reductions using 150, 300, and 600\,arcsec filter edges
in the reductions. The synthetic data are clearly seen as the circular
region S.E. of M17. As the filter scale is increased, the size of the
ripples increases accordingly. While larger structures do seem to
appear, negative bowls are a major problem without any other map
...
One fairly arbitrary parameter in our data reduction is the choice of
angular scale for the \model{FLT} filter. The current default of
300\,arcsec\ was chosen ``by-eye'' as a compromise that would provides
aesthetically reasonable results for a range of observing
modes. However, there is a simple tradeoff that warrants
experimentation for specific fields. By choosing a larger scale (lower
frequency in the bolometers signals) for the filter, potentially more
structure will be faithfully re-produced in the map (up to the scale
of the array footprint, at which point the common-mode removal will
dominate). However, lower the filter edge can potentially let through
more independent low-frequency noise. We have therefore re-run the
previous tests using two other scales for the filters, and show the
resulting maps and angular SNR spectra in Figs ... .

Note that alternatives to the zero-masking approach do exist for other
iterative map-makers. For example, \citet{kovacs2008} restricts the
solution to a small number of iterations to avoid divergence on
large-scales. For BOLOCAM data of the Galactic Plane,
\citet{aguirre2010} used a Maximum-Entropy filtering step to suppress
large-scales. The zero-masking implemented in SMURF is similar in
principle to the \citet{aguirre2010} method, while being slightly
conceptually simpler. However, since both methods are non-linear,
detailed simulations would be required to determine their
SNR-dependent angular transfer functions.
