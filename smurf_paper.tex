...
time. Given the massive volume of \scuba\ data produced in a single
observation (a rate of approximately 10 GB\,hr$^{-1}$ in each focal
plane), iterative techniques were the only viable option for providing
real-time feedback at the telescope, and for automatically reducing
near-science grade maps on a timescale comensurate with the
observation times using standard high-end desktop computers.  We do
note, however, that recent advances in direct inversion methods such
as SANEPIC \citep{patanchon2008} may in fact be tenable for \scuba\
data. A future goal of the software team is to investigate meshing
SMURF (perhaps as a cleaning step and initial condition) with a
SANEPIC-like algorithm.
...
framework with a simple interface for user configurability. To achieve
this goal, while minimizing development time, it was decided to create
SMURF as a Starlink package \citep{2009ASPC..411..418J}, which
provides access to a large suite of libraries (including a commanding
and messaging interface, astrometric conversions and generation of
...
interoperability. Though originally written primarily in FORTRAN, many
of the core Starlink libraries are now ported into native C, or else
have, at least, a C interface. It was therefore decided to develop
SMURF in C (rather than C++ which would have required adding further
dependencies to Starlink), although the noise models are implemented
with an object-oriented approach (all data associated with models are
stored in structures, and there is a standard interface for method
functions). In this way it is fairly easy to add new
models. Parallelisation is incorporated in the most time-consuming
low-level routines (e.g. performing Fast Fourier Transforms,
re-gridding the data), usually handling either independent blocks of
bolometers, or blocks of time, in each thread, depending on the nature
of the calculation. ... something about scalability ...
