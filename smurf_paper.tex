...
and subtend a nearly contiguous 7\,arcmin $\times$ 7\,arcmin footprint
on the sky.  The instrument is also equipped with a Fourier transform
spectrometer \citep[FTS-2,][]{gom2010}, and a polarimeter
\cite[POL-2,][]{2011ASPC..449...68B}. This paper describes the
properties of \scuba\ data that are relevant to producing maps of the
imaging data, and the Submillimetre User Reduction Facility, SMURF, a
software package for perfoming the reduction written using the
Starlink Software Environment \citep{2009ASPC..411..418J}. The
...
$T_\mathrm{C}$. The TESs then have small electrical voltage biases,
...
noted that there is an additional 33rd row of SQUID readouts that are
...
manufacturing differences, considerable effort has gone into
dynamically applying position-dependent biases as well as a total
focal plane bias to make as many detectors usable as possible
...
the detector, the spectral response of the bolometers (bandpass), the
spectrum of the source, and additional optical efficiency factors -- a
\emph{flux conversion factor} (FCF). Extinction is measured, in most
cases, using the JCMT line-of-sight water vapour radiometer (WVM)
which is calibrated using sky-dips and the CSO 225\,GHz tipper. The
FCF is determined empirically using measurements of standard
sources. For further details on these calibration steps, see
\citet{dempsey2012}.
...
than the chop throw (a maximum of several arcmin). The primary
approach to map-making with \scuba\ is instead to slew the entire
telescope at such a rate that angular scales of interest are crossed
on time scales shorter than $1/f_\mathrm{knee}$, similar to other
ground-based instruments (MAMBO, BOLOCAM, SHARC-II, AzTEC,
LABOCA,...). The desire to scan rapidly has two basic constraints: (i)
the telescope cannot physically move faster that several hundred
arcsec\,sec$^{-1}$ without requiring unacceptably large accelerations
to change direction; and (ii) bolometers are designed with a thermal
time constant that balances response with sensitivity. Taking these
constraints into consideration, \scuba\ was ultimately designed to
work at a maximum scan speed of 600\,arcsec\,sec$^{-1}$. The data
acquisition system has a target sample rate of 200\,Hz, which at that
scan velocity results in a sample every 3.0\,arcsec, giving
approximately 2.5 samples per 450\,\micron\ diffraction-limited
full-width at half-maximum (FWHM) -- a typical rule-of-thumb for
adequately sampling a Gaussian point spread function (PSF). Similarly,
the \scuba\ bolometers are designed to have a time constant of 2 ms
\textbf{check that number!} in order to avoid distorting PSF-sized
sources at this maximum scan rate.
...
\citep[e.g.][]{tegmark1997,kovacs2008b}; such strategies are said to
offer good \emph{cross-linking}. As a tradeoff between truly random
scan patterns and feasibility of implementation taking into
consideration both mechanical and scheduling constraints, \scuba
...
domain requires an additional multiplcation by 2; hence the units are
...
\caption{Bolometer power spectral densities. Four representative
  ...
  scales are comparable to the array footprint.}
\label{fig:pspec}
\end{figure*}
...
noise. Finally, we note that the amplitudes and slopes of
...
the dark squid, leading to a significantly higher \snr.
...
that the absolute azimuth of the telescope was close to 180$^\circ$
...
there were problems with the subarray fabrication process, which led
to large numbers of dead pixels, as well as large-scale variations in
...
in direct inversion methods such as SANPIEC \citep{patanchon2008} may
...
the time but suffer from transient problems at specific times.
...
differences, as described in \citet{west1979}.
 ...
 We decided not to use the \emph{unbiased} estimator, since in practice
it would require accumulating an additional array of values at every
map pixel, and only results in a small difference where there are less
