...
\begin{abstract}
  ...
  scanned across the sky and records data at a rate of 200\,Hz. The
  data rate is much greater than that of previously existing
...
is typically dominated by sources which produce slow variations the
...
This paper is organized in the following way. We first describe the
properties of \scuba\ data in Section~\ref{sec:data} to motivate the
data model that we have adopted. Next, the details of the SMURF
...
In this section we summarize how \scuba\ works,
(Section~\ref{sec:bolos}), give exampels of typical bolometers signals
...
populated by four \emph{subarrays} (labeled s4a--s4d at 450\,\micron,
and s8a--s8d at 850\,\micron), each consisting of 32 columns and 40
rows of bolometers. The bolometers are thermal absorbers coupled to
...
digitized. Each detector has its own SQUID for the first-stage of the
...
connected to TESs. These \emph{dark SQUIDs} track non-thermal noise
...
power, $P$, or $dI/dP$ is established using \emph{flatfield}
...
\citet{dempsey2012} and Section~5.3 in \citet{holland2012}. Finally,
the conversion to astronomical flux units from power involves a
correction for atmospheric extinction \citep[primarily using the JCMT
Water Vapour Monitor to track line-of-sight opacity variations, see
...
signature from other common signals that appear with the same sign.
...
\caption{Typical map-making algorithm. Raw data (stored in multiple
  ...
  back into the time-series (and importantly, \emph{not} the other
  components).}
\label{fig:dimm}
\end{figure}
...
to iterate this process along with estimates of the map. This sort of
algorithm has been used successfully in the past with instruments
including SCUBA \citep{1999ASPC..172..171J}, SHARC-2
\citep{kovacs2008}, and BOLOCAM \citep{aguirre2010}, and is capable of
recovering modest angular scales up to of order the footprint of the
array (depending, of course, on the noise in the data). The main
benefit of this approach over CMB-style maximum-likelihood methods
(which are generally superior at recovering structure on all scales
available in the data) is that the computing resources are
significantly smaller, both in terms of memory usage and execution
time. Given the massive volume of \scuba\ data (produced at a rate of
approximately 10 GB\,hr$^{-1}$ in each focal plane), iterative
techniques were the only viable option for providing real-time
feedback at the telescope, and for automatically reducing near-science
grade maps on a timescale comensurate with the observation times,
using single high-end desktop computers.  We do note, however, that
recent advances in direct inversion methods such as SANEPIC
\citep{patanchon2008} may in fact be tenable for \scuba\ data. A
future goal of the software team is to investigate meshing SMURF
(perhaps as a cleaning step and initial condition) with a SANEPIC-like
algorithm.

Significant experimentation with different models for noise sources
during \scuba\ commissioning required a highly flexible software
framework, and a configurable user interface. To achieve this goal,
while minimizing development time, it was decided to build SMURF as a
Starlink package \citep{2009ASPC..411..418J}, which provides access to
a large suite of libraries (including a commanding and messaging
interface, astrometric coordinate conversions and generation of
standard WCS information, file formats etc.). Furthermore, Starlink is
open source, and already used extensively at the Joint Astronomy
Centre (host of the JCMT) for many other systems, which helps with
...
thread, depending on the nature of the
calculation. \textbf{... something about scalability ...}
...
a second dashed box are \emph{multiplicative} rather than additive
models, and so they must be inverted prior to the start of each
iteration to preserve the units (see Sections~\ref{sec:comgai} and
\ref{sec:ext}).
...
for the full length of each continuousy chunk of data. Given the
...
estimation. In practice, map-based despiking usually gives superior
results, and so time-domain despiking is switched off by default.
...
$\mathbf{n}^\mathrm{c}(t)$ is a correlated or \emph{common-mode}
signal (with an optional scale factor $g_i$ for each bolometer), and
...
are the only components of the residual bolometer signal, regridding
...
instant in time), leading to an \emph{over-subtraction}, and in turn,
...
degenerices, see Section~\ref{sec:converge}.
...
used purely as a high-pass filter to remove most of the residual
uncorrelated $1/f$ noise following common-mode removal
